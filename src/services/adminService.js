...
class AdminService {
		...
		try {
			...
			for (const pkg of servicePackages) {
				...
				const data = {
					...
					is_active:
						pkg.is_active !== undefined ? pkg.is_active : true,
					carry_on_baggage: pkg.carry_on_baggage || 7,
					checked_baggage: pkg.checked_baggage || 20,
					refund_percentage: pkg.refund_percentage || 0,
					travel_insurance: pkg.travel_insurance || false,
			...
			for (const e of existing) {
				const keepById =
					e.package_id && incomingIds.includes(e.package_id);
				const keepByCode =
		...
		try {
			const defaultPackages = [
				...
				},
				{
					...
					services_included:
						'Extra legroom, priority boarding, meal service',
					carry_on_baggage: 7,
					checked_baggage: 25,
				...
				},
				{
					...
					services_included:
						'Premium seat, lounge access, meal service',
					carry_on_baggage: 7,
					checked_baggage: 32,
				...
				},
				{
					...
					services_included:
						'All business services plus premium amenities',
					carry_on_baggage: 7,
					checked_baggage: 40,
				...
				for (const flightItem of flights) {
					const selectedPackages =
						await BookingServicePackage.findAll({
							...
							include: [
								{ model: ServicePackage, required: true },
							],
						});
			...
			const airlineStats = await Booking.findAll({
				attributes: [
					[
						sequelize.fn(
							'COUNT',
							sequelize.col('Booking.booking_id')
						),
						'booking_count',
					],
					[
			...
			const baggageStats = await BookingDetail.findAll({
				attributes: [
					...
					[
						sequelize.fn(
							'SUM',
							sequelize.col('BaggageOption.price')
						),
						'total_revenue',
					],
				],
		...
		try {
			...
			const airlineStats = await BookingDetail.findAll({
				attributes: [
					...
					[
						sequelize.col('Flight.Airline.airline_name'),
						'airline_name',
					],
					[
						sequelize.col('Flight.Airline.airline_code'),
						'airline_code',
					],
				],
				include: [
	...
	async getBaggageServiceStatistics(period = 'month') {
		try {
			...
			const totalRevenue = baggageStats.reduce(
				(sum, stat) =>
					sum + parseFloat(stat.dataValues.total_revenue || 0),
				0
			);
	...
	async updateTravelClass(id, updateData) {
		try {
			...
			if (updateData.class_code) {
				...
				if (existingTravelClass) {
					throw new BadRequestError(
						'Travel class code already exists'
					);
				}
			}
	...
	async getBaggageOptions(filters = {}, page = 1, limit = 10) {
		try {
			...
			return await this.baggageOptionService.findAll(
				options,
				page,
				limit
			);
	...
	async createBaggageOption(baggageOptionData) {
		try {
			// Verify airline exists
			const airline = await Airline.findByPk(
				baggageOptionData.airline_id
			);
			if (!airline) {
				throw new BadRequestError('Airline not found');
			}
			...
			const enhancedFlights = await Promise.all(
				result.data.map(async (flight) => {
					...
					const isDomestic =
						depCountryId &&
						arrCountryId &&
						depCountryId === arrCountryId;
					const flightTypeDisplay = isDomestic
						? 'Nội địa'
						: 'Quốc tế';
	...
	buildFlightWhereClause(filters) {
		...
		if (filters.arrival_time_from || filters.arrival_time_to) {
			...
			if (filters.arrival_time_from) {
				whereClause.arrival_time[Op.gte] = new Date(
					filters.arrival_time_from
				);
			}
			if (filters.arrival_time_to) {
				whereClause.arrival_time[Op.lte] = new Date(
					filters.arrival_time_to
				);
			}
		}
	...
	async createFlight(flightData) {
		try {
			...
			if (
				...
				flightData.meal_services.length > 0
			) {
	...
	async createFlightBaggageService(flightId, baggageServiceData) {
		try {
			...
			const baggageService = await FlightBaggageService.create(
				serviceData
			);
	...
	async syncFlightSeats(flightId, options = { force: false }) {
		try {
			...
			if (!businessClass || !economyClass) {
				throw new Error(
					'Travel classes BUSINESS and ECONOMY must exist'
				);
			}
			...
			if (businessCount + economyCount !== totalSeatsDeclared) {
				const adjustedEconomy = Math.max(
					0,
					totalSeatsDeclared - businessCount
				);
				logger.warn(
	...
	async generateFlightSeats(flightId, aircraft) {
		try {
			...
			logger.info(
				`Generated ${seats.length} seats for flight ${flightId}`
			);
		} catch (error) {
	...
	async updateFlight(flightId, updateData) {
		try {
			...
			const departureId =
				updateData.departure_airport_id ||
				existingFlight.departure_airport_id;
			const arrivalId =
				updateData.arrival_airport_id ||
				existingFlight.arrival_airport_id;
			...
			if (
				...
				const duplicateFlight = await Flight.findOne({
					where: {
						...
						airline_id:
							updateData.airline_id || existingFlight.airline_id,
				...
				if (Array.isArray(updateData.baggage_services)) {
					...
					for (const item of requested) {
						if (item.baggage_service_id) {
							// update existing
							const rec = existingById.get(
								item.baggage_service_id
							);
							if (rec) {
								await rec.update(
									{
